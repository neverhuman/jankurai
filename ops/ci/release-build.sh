...
dist="${CI_ROOT}/dist"
# BUILD_DIR is where the cargo workspace lives. In jankurai-core/deploy it equals
# CI_ROOT; on the public hub it is the fused workspace (.fusion) assembled from the
# published tool-crate tags by scripts/fuse.sh. Default keeps backward-compat.
build_dir="${BUILD_DIR:-${CI_ROOT}}"
binary="${build_dir}/target/${target}/release/jankurai"
...
( cd "${build_dir}" && cargo build --release --locked -p jankurai --target "${target}" )
