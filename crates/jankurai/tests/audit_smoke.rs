...
fn changed_fast_inventories_unchanged_generic_supporting_evidence() {
    ...
    .unwrap();
    fs::create_dir_all(dir.path().join("ops/ci")).unwrap();
    fs::write(
        dir.path().join("ops/ci/split-host-ci-parent.sh"),
        "#!/bin/sh\nset -eu\ncargo check\n",
    )
    .unwrap();
    ...
    fs::write(
        dir.path().join("contracts/generated/event.schema.json"),
    ...
    fs::write(
        dir.path().join("schemas/repo-score.schema.json"),
        "{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\"}\n",
    )
    .unwrap();
    fs::write(
        dir.path().join("schemas/repair-queue.schema.json"),
        "{\"$schema\":\"https://json-schema.org/draft/2020-12/schema\"}\n",
    )
    .unwrap();
