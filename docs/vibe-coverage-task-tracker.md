...
| VCOV-05 | Audit integration | done | `model.rs`, `audit/mod.rs`, `render.rs`, repo-score schema | `cargo test -p jankurai --test report_compatibility_guard` | `agent/repo-score.*` | optional summary must not invalidate old reports |
