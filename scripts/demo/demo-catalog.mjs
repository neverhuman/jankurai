...
export const CAPTURE_FILES = Object.freeze({
  'audit-recording.json': 'recording.json',
  'report.json': 'report.json',
  'stdout.txt': 'stdout.txt',
  'stderr.txt': 'stderr.txt',
  'sample-inputs.json': 'sample-inputs.json',
  'producer-audit-state.json': 'producer-audit-state.json',
});
export const PUBLIC_FILES = Object.freeze([...PRESETS.map((preset) => preset.name), RECEIPT, ...Object.keys(CAPTURE_FILES)]);
