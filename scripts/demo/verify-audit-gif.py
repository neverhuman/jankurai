...
for output in manifest['outputs']:
    ...
    with Image.open(file) as image:
        ...
        for index, expected in enumerate(output['frames']):
            ...
            counts = rgb.getcolors(maxcolors=len(palette))
            assert counts is not None and all(color in palette for _, color in counts), (file, index, 'unexpected dimmed/quantized color')
        if '--read-only' not in sys.argv[2:]:
